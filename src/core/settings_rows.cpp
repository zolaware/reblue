...
// The cheat page reads left-to-right from disabled to strongest, so its
// booleans put Off first. kOnOff is the opposite order and is shared by every
// other page, so it is left alone rather than flipped under them.
constexpr SettingOption kOffOn[] = {
    {.text = "Off", .num = 0, .value = "false", .key = "opt.off"},
    {.text = "On", .num = 1, .value = "true", .key = "opt.on"}};

// The one row that is an action rather than a state. "+1" reads as something
// applied once, which is what it is: the row fires and puts itself back to Off,
// so an On/Off pair would sit there claiming a state it does not hold.
constexpr SettingOption kActionOnce[] = {
    {.text = "Off", .num = 0, .value = "false", .key = "opt.off"},
    {.text = "+1", .num = 1, .value = "true"}};

// Multiplier steps, not free values: a menu row cycles a fixed list, and these
// are the ones worth offering. The cvar behind each row still takes anything.
constexpr SettingOption kCheatMult[] = {
    {.text = "Off", .num = 1, .key = "opt.off"},
    {.text = "2x", .num = 2},
    {.text = "5x", .num = 5},
    {.text = "10x", .num = 10}};
constexpr SettingOption kRewardMult[] = {
    {.text = "Off", .num = 1, .key = "opt.off"},
    {.text = "2x", .num = 2},
    {.text = "3x", .num = 3},
    {.text = "5x", .num = 5}};
constexpr SettingOption kStatBonus[] = {
    {.text = "Off", .num = 0, .key = "opt.off"},
    {.text = "+999", .num = 999},
    {.text = "+9999", .num = 9999}};

// Every row here writes through the engine layer rather than at guest memory
// directly, so a pin is a value the game could have stored itself.
constexpr SettingRow kCheatSettings[] = {
    {.label = "settings.cheats.invincible.label",
     .group = "menu.header.cheats_battle",
     .binding = {.get = [] { return engine::Cheats::Get().Invincible() ? 1.0 : 0.0; },
                 .set = [](double v) { return engine::Cheats::Get().SetInvincible(v != 0.0); }},
     .options = kOffOn,
     .count = OptCount(kOffOn)},
    {.label = "settings.cheats.infinite_mp.label",
     .group = "menu.header.cheats_battle",
     .binding = {.get = [] { return engine::Cheats::Get().InfiniteMP() ? 1.0 : 0.0; },
                 .set = [](double v) { return engine::Cheats::Get().SetInfiniteMP(v != 0.0); }},
     .options = kOffOn,
     .count = OptCount(kOffOn)},
    {.label = "settings.cheats.status_immune.label",
     .group = "menu.header.cheats_battle",
     .binding = {.get = [] { return engine::Cheats::Get().StatusImmune() ? 1.0 : 0.0; },
                 .set = [](double v) { return engine::Cheats::Get().SetStatusImmune(v != 0.0); }},
     .options = kOffOn,
     .count = OptCount(kOffOn)},
    {.label = "settings.cheats.one_hit_kill.label",
     .group = "menu.header.cheats_battle",
     .binding = {.get = [] { return engine::Cheats::Get().OneHitKill() ? 1.0 : 0.0; },
                 .set = [](double v) { return engine::Cheats::Get().SetOneHitKill(v != 0.0); }},
     .options = kOffOn,
     .count = OptCount(kOffOn)},
    {.label = "settings.cheats.attack_mult.label",
     .group = "menu.header.cheats_power",
     .binding = {.get = [] { return static_cast<double>(engine::Cheats::Get().AttackMult()); },
                 .set = [](double v) { return engine::Cheats::Get().SetAttackMult(static_cast<i32>(v)); }},
     .options = kCheatMult,
     .count = OptCount(kCheatMult)},
    {.label = "settings.cheats.magic_attack_mult.label",
     .group = "menu.header.cheats_power",
     .binding = {.get = [] { return static_cast<double>(engine::Cheats::Get().MagicAttackMult()); },
                 .set = [](double v) { return engine::Cheats::Get().SetMagicAttackMult(static_cast<i32>(v)); }},
     .options = kCheatMult,
     .count = OptCount(kCheatMult)},
    {.label = "settings.cheats.defence_mult.label",
     .group = "menu.header.cheats_power",
     .binding = {.get = [] { return static_cast<double>(engine::Cheats::Get().DefenceMult()); },
                 .set = [](double v) { return engine::Cheats::Get().SetDefenceMult(static_cast<i32>(v)); }},
     .options = kCheatMult,
     .count = OptCount(kCheatMult)},
    {.label = "settings.cheats.magic_defence_mult.label",
     .group = "menu.header.cheats_power",
     .binding = {.get = [] { return static_cast<double>(engine::Cheats::Get().MagicDefenceMult()); },
                 .set = [](double v) { return engine::Cheats::Get().SetMagicDefenceMult(static_cast<i32>(v)); }},
     .options = kCheatMult,
     .count = OptCount(kCheatMult)},
    {.label = "settings.cheats.agility_mult.label",
     .group = "menu.header.cheats_power",
     .binding = {.get = [] { return static_cast<double>(engine::Cheats::Get().AgilityMult()); },
                 .set = [](double v) { return engine::Cheats::Get().SetAgilityMult(static_cast<i32>(v)); }},
     .options = kCheatMult,
     .count = OptCount(kCheatMult)},
    {.label = "settings.cheats.stat_bonus.label",
     .group = "menu.header.cheats_power",
     .binding = {.get = [] { return static_cast<double>(engine::Cheats::Get().StatBonus()); },
                 .set = [](double v) { return engine::Cheats::Get().SetStatBonus(static_cast<i32>(v)); }},
     .options = kStatBonus,
     .count = OptCount(kStatBonus)},
    {.label = "settings.cheats.exp_mult.label",
     .group = "menu.header.cheats_progress",
     .binding = {.get = [] { return static_cast<double>(engine::Cheats::Get().ExpMult()); },
                 .set = [](double v) { return engine::Cheats::Get().SetExpMult(static_cast<i32>(v)); }},
     .options = kRewardMult,
     .count = OptCount(kRewardMult)},
    {.label = "settings.cheats.sp_mult.label",
     .group = "menu.header.cheats_progress",
     .binding = {.get = [] { return static_cast<double>(engine::Cheats::Get().SpMult()); },
                 .set = [](double v) { return engine::Cheats::Get().SetSpMult(static_cast<i32>(v)); }},
     .options = kRewardMult,
     .count = OptCount(kRewardMult)},
    {.label = "settings.cheats.gold_mult.label",
     .group = "menu.header.cheats_progress",
     .binding = {.get = [] { return static_cast<double>(engine::Cheats::Get().GoldMult()); },
                 .set = [](double v) { return engine::Cheats::Get().SetGoldMult(static_cast<i32>(v)); }},
     .options = kRewardMult,
     .count = OptCount(kRewardMult)},
    {.label = "settings.cheats.medals_mult.label",
     .group = "menu.header.cheats_progress",
     .binding = {.get = [] { return static_cast<double>(engine::Cheats::Get().MedalsMult()); },
                 .set = [](double v) { return engine::Cheats::Get().SetMedalsMult(static_cast<i32>(v)); }},
     .options = kRewardMult,
     .count = OptCount(kRewardMult)},
    {.label = "settings.cheats.infinite_gold.label",
     .group = "menu.header.cheats_progress",
     ...
     .options = kOffOn,
     .count = OptCount(kOffOn)},
    {.label = "settings.cheats.unlock_classes.label",
     .group = "menu.header.cheats_progress",
     .binding = {.get = [] { return engine::Cheats::Get().UnlockClasses() ? 1.0 : 0.0; },
                 .set = [](double v) { return engine::Cheats::Get().SetUnlockClasses(v != 0.0); }},
     .options = kOffOn,
     .count = OptCount(kOffOn)},
    {.label = "settings.cheats.give_all_items.label",
     .group = "menu.header.cheats_items",
     .binding = {.get = [] { return engine::Cheats::Get().GiveAllItems() ? 1.0 : 0.0; },
                 .set = [](double v) { return engine::Cheats::Get().SetGiveAllItems(v != 0.0); }},
     .options = kActionOnce,
     .count = OptCount(kActionOnce)},
    {.label = "settings.cheats.give_heal.label",
     .group = "menu.header.cheats_items",
     .binding = {.get = [] { return engine::Cheats::Get().Grant(engine::ItemCategory::kHeal) ? 1.0 : 0.0; },
                 .set = [](double v) { return engine::Cheats::Get().SetGrant(engine::ItemCategory::kHeal, v != 0.0); }},
     .options = kActionOnce,
     .count = OptCount(kActionOnce)},
    {.label = "settings.cheats.give_usable.label",
     .group = "menu.header.cheats_items",
     .binding = {.get = [] { return engine::Cheats::Get().Grant(engine::ItemCategory::kUsable) ? 1.0 : 0.0; },
                 .set = [](double v) { return engine::Cheats::Get().SetGrant(engine::ItemCategory::kUsable, v != 0.0); }},
     .options = kActionOnce,
     .count = OptCount(kActionOnce)},
    {.label = "settings.cheats.give_spellbook.label",
     .group = "menu.header.cheats_items",
     .binding = {.get = [] { return engine::Cheats::Get().Grant(engine::ItemCategory::kSpellbook) ? 1.0 : 0.0; },
                 .set = [](double v) { return engine::Cheats::Get().SetGrant(engine::ItemCategory::kSpellbook, v != 0.0); }},
     .options = kActionOnce,
     .count = OptCount(kActionOnce)},
    {.label = "settings.cheats.give_arm.label",
     .group = "menu.header.cheats_items",
     .binding = {.get = [] { return engine::Cheats::Get().Grant(engine::ItemCategory::kArm) ? 1.0 : 0.0; },
                 .set = [](double v) { return engine::Cheats::Get().SetGrant(engine::ItemCategory::kArm, v != 0.0); }},
     .options = kActionOnce,
     .count = OptCount(kActionOnce)},
    {.label = "settings.cheats.give_finger.label",
     .group = "menu.header.cheats_items",
     .binding = {.get = [] { return engine::Cheats::Get().Grant(engine::ItemCategory::kFinger) ? 1.0 : 0.0; },
                 .set = [](double v) { return engine::Cheats::Get().SetGrant(engine::ItemCategory::kFinger, v != 0.0); }},
     .options = kActionOnce,
     .count = OptCount(kActionOnce)},
    {.label = "settings.cheats.give_ear.label",
     .group = "menu.header.cheats_items",
     .binding = {.get = [] { return engine::Cheats::Get().Grant(engine::ItemCategory::kEar) ? 1.0 : 0.0; },
                 .set = [](double v) { return engine::Cheats::Get().SetGrant(engine::ItemCategory::kEar, v != 0.0); }},
     .options = kActionOnce,
     .count = OptCount(kActionOnce)},
    {.label = "settings.cheats.give_neck.label",
     .group = "menu.header.cheats_items",
     .binding = {.get = [] { return engine::Cheats::Get().Grant(engine::ItemCategory::kNeck) ? 1.0 : 0.0; },
                 .set = [](double v) { return engine::Cheats::Get().SetGrant(engine::ItemCategory::kNeck, v != 0.0); }},
     .options = kActionOnce,
     .count = OptCount(kActionOnce)},
    {.label = "settings.cheats.give_chest.label",
     .group = "menu.header.cheats_items",
     .binding = {.get = [] { return engine::Cheats::Get().Grant(engine::ItemCategory::kChest) ? 1.0 : 0.0; },
                 .set = [](double v) { return engine::Cheats::Get().SetGrant(engine::ItemCategory::kChest, v != 0.0); }},
     .options = kActionOnce,
     .count = OptCount(kActionOnce)},
    {.label = "settings.cheats.give_valuable.label",
     .group = "menu.header.cheats_items",
     .binding = {.get = [] { return engine::Cheats::Get().Grant(engine::ItemCategory::kValuable) ? 1.0 : 0.0; },
                 .set = [](double v) { return engine::Cheats::Get().SetGrant(engine::ItemCategory::kValuable, v != 0.0); }},
     .options = kActionOnce,
     .count = OptCount(kActionOnce)},
    {.label = "settings.cheats.infinite_items.label",
     .group = "menu.header.cheats_items",
     .binding = {.get = [] { return engine::Cheats::Get().InfiniteItems() ? 1.0 : 0.0; },
                 .set = [](double v) { return engine::Cheats::Get().SetInfiniteItems(v != 0.0); }},
     .options = kOffOn,
     .count = OptCount(kOffOn)},

    {.label = "settings.cheats.unlock_achievements.label",
     .group = "menu.header.cheats_encyclopedia",
     .binding = {.get = [] { return engine::Cheats::Get().UnlockAchievements() ? 1.0 : 0.0; },
                 .set = [](double v) { return engine::Cheats::Get().SetUnlockAchievements(v != 0.0); }},
     .options = kActionOnce,
     .count = OptCount(kActionOnce)},
    {.label = "settings.cheats.reset_achievements.label",
     .group = "menu.header.cheats_encyclopedia",
     .binding = {.get = [] { return engine::Cheats::Get().ResetAchievements() ? 1.0 : 0.0; },
...
constexpr SettingsPageTable kPages[kSettingsPageCount] = {
    ...
    {"settings.page.cheats", kCheatSettings,
     static_cast<int>(std::size(kCheatSettings))},
